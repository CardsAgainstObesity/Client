...
export default {
	data(){
		return {
			chapuza: `<span class='card_input'>[...]</span>`,
		}
	},
    props: {
        text: {
	...
	methods: {
		assignInput(value, clickable, language) {
			if (clickable) {
				if (
					document.querySelectorAll(".card_input").length <= game.card_index
				) {
					game.card = 0;
				} else {
					document.querySelectorAll(".card_input")[
						game.card_index
					].innerHTML = value[language].replaceAll(".", "");
...
<template>
    <div
        @click="assignInput(text, clickable, $display.language)"
        :class="
            'game-card ' +
            (dark ? 'dark ' : ' ') +
            (clickable ? 'clickable ' : ' ') +
